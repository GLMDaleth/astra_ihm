  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = mqtt_client_interfaces__msg__RosMsgType;
    is_plain =
      (
      offsetof(DataType, stamped) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _RosMsgType__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_mqtt_client_interfaces__msg__RosMsgType(
