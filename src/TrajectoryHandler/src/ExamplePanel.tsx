...
function ZoneScenarioPanel({ context }: { context: PanelExtensionContext }) {
  ...
  const advertisedRef = useRef({ nearest: false, reverse: false , reset:false});
  ...
  const advertiseSomeTopics = (key: "reset" , topic: string) => {
    if (!advertisedRef.current[key] && context.advertise) {
      context.advertise(topic, "std_msgs/Bool");
      advertisedRef.current[key] = true;
    }
  ...
  useEffect(() => {
    ...
    advertiseSomeTopics("reset","/reload_scenarios_db");
  }, [context]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <PressableButton style={greenButtonStyle} onClick={() => publishBoolTopic("/reload_scenarios_db", true)}>
          Reload Missions
        </PressableButton>
